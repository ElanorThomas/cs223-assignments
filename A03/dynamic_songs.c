...
void printSongs(int songArraySize, struct Song givenSongs[]){
    ...
    for (i = 0; i < songArraySize; i++){
        ...
        i, givenSongs[i].songTitle, givenSongs[i].artist,
        minutes, seconds, givenSongs[i].danceability,
        givenSongs[i].energy, givenSongs[i].tempo,
        givenSongs[i].valence);
    }
}
...
int main() {
    ...
    fgets(lineRead, 300, filePtr);//this is the header line, ignor  
    int i;
    for (i = 0; i < numSongs; i++){
        fgets(lineRead, 300, filePtr);
        char *curTit = strtok(lineRead, s);
        char *curArt = strtok(NULL, s);
        struct Song curSong = {"placeholder", "placeholder",
        atoi(strtok(NULL, s)), atof(strtok(NULL, s)),
        atof(strtok(NULL, s)), atof(strtok(NULL, s)),
        atof(strtok(NULL, s))};
